...
- W3:routePeers/rawRoutePeerInfos/connBitmapPeerIds/connBitmapEdges/PeerCenter 关键观测状态写入 DO storage;DO alarm 做 10s Ping、25s 超时、TTL stale cleanup。WebSocket 会话本身仍是运行时态;尚未迁移到 Cloudflare WebSocket Hibernation API。
- W4:Topology DTO 增加 `summary`(节点/边/latency/PeerCenter ratio),面板直接消费 summary。现已继续扩展为:
  `TrafficSnapshot.samples/summary`(Worker 可观测 RX/TX 速率折线图),
  `TopologySnapshot.routes`(以 Worker 为根、由 conn bitmap/live peer 推导的路由路径),
  `TopologySnapshot.connectionMatrix`(conn bitmap 矩阵),Topology 页展示 SVG 连接图、route table、bitmap matrix。
  `relayDropRate` 明确是 Worker invalid/unroutable 帧比例;官方 EasyTier `loss_rate` 属
  `api_instance.PeerConnInfo.loss_rate`,当前 peer RPC 控制面未提供时必须显示未观测,不得冒充。
- W6:新增 `EASYTIER_NETWORKS` per-room network map,并兼容 `EASYTIER_NETWORK_SECRETS`/`EASYTIER_NETWORK_SECRET` fallback。
- W5:outbound TCP 已实现并部署验证。官方 TCP tunnel 确认为 `u32 little-endian length + EasyTier 16B peer-manager frame`;`RelayRoom` 可用 `cloudflare:sockets` 主动拨 `tcp://` 公共节点、发送客户端握手、复用现有解密/RPC/拓扑观测路径。部署版 `6a15c4e3-3582-472d-9019-73b8147ed2ed` 已主动接入 `home-mesh` 并解出真实拓扑。
...
- 已补:Overview 流量折线图与 RX/TX 速率;Devices 节点详情/表格显示 PeerCenter 延迟、连接速率和可选 loss rate;
  Topology 显示连接图、Worker-rooted route path、conn bitmap matrix、PeerCenter 延迟边。
- 边界:上传/下载速率来自经过 Worker 的 WSS/TCP 会话计数;节点间直接 P2P 数据面不经过 Worker 时不可见。
  官方丢包率需 EasyTier 管理 API `ListPeer/ListRoute` 或等价字段,未解码到该来源时 UI 显示未观测。
...
### W5 — 出站并网:`connect()` 拨现有公共节点（已实现并部署实测）
现状:`src/easytier/tcp-frame.ts` 实现官方 TCP tunnel 帧;`RelayRoom` 支持 outbound TCP session。
- TCP wire:`4-byte little-endian payload length` + `payload`。payload 是现有 EasyTier peer-manager frame(16B header + body);官方 2.6.4 `FramedReader` 单帧上限 2000B。
- 配置:
  - `EASYTIER_PUBLIC_PEER_TCP`:单网络 fallback,可设为 `tcp://host:port`。
  - `EASYTIER_OUTBOUND_TCP_PEERS`:多 room 配置,支持 JSON array / per-room map / comma-newline 列表。
- 生命周期:房间 API/面板访问会触发已配置 peer 的主动拨号;DO alarm 做心跳/断线重连;`GET|POST /api/rooms/:roomId/outbound-tcp` 可查看/触发状态。
- 协议路径:Worker 作为 TCP client 先发 HandshakeRequest(`from=EDGE_PEER_ID,to=0,type=2`),收到服务端 HandshakeRequest 后校验 magic/version/network/digest,再复用 AES-GCM 解密、RPC decode、route push/PeerCenter/topology。
- 限制:仅 `tcp://`;Workers 仍不能拨 `udp://`、不能 TUN/L3。大于 2000B 的单个 EasyTier TCP payload 会被拒绝,需后续按官方分片/压缩行为补 live evidence。
- 验收:部署 Worker secret 后,不改任何真实节点配置,EdgeTier 主动并入组网并解出全网信息。已在 `home-mesh` 验证:outbound TCP `handshakeAccepted=true`,远端 peer `home-kwrt`,拓扑 6 nodes / 16 edges。
...
5. 继续 W7 真机长测;W5 outbound TCP 已具备 Worker 实现和 Cloudflare live evidence。
